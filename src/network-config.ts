...
import { ArchaeologistFacet__factory, EmbalmerFacet, EmbalmerFacet__factory, IERC20, IERC20__factory, ThirdPartyFacet, ThirdPartyFacet__factory, ViewStateFacet, ViewStateFacet__factory } from "@sarcophagus-org/sarcophagus-v2-contracts";
import { 
  SarcoNetworkConfig, 
  goerliNetworkConfig, 
...
const getNetworkContextByChainId = (chainId: number, isTest: boolean): NetworkContext => { 
  ...
  const getNetworkConfig: NetworkConfigReturningFunction = chainIdsToNetworkConfigReturningFunction[chainId];
  ...
  const archaeologistFacet = ArchaeologistFacet__factory.connect(networkConfig.diamondDeployAddress, ethWallet);
  const viewStateFacet = ViewStateFacet__factory.connect(networkConfig.diamondDeployAddress, ethWallet);
  const thirdPartyFacet = ThirdPartyFacet__factory.connect(networkConfig.diamondDeployAddress, ethWallet);
  const embalmerFacet = EmbalmerFacet__factory.connect(networkConfig.diamondDeployAddress, ethWallet);
...
export const getNetworkContextsByChainIds = (chainIds: number[], isTest: boolean): NetworkContext[] => {
  if (!chainIds.length) {
